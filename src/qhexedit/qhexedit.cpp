...
void QHexEdit::setCursorPosition(qint64 position) {
  ...
  if (_overwriteMode)
    _cursorRect =
        QRect(_pxCursorX - horizontalScrollBar()->value(), _pxCursorY + _pxCursorWidth, _pxCharWidth, _pxCursorWidth);
  else
    _cursorRect = QRect(_pxCursorX - horizontalScrollBar()->value(),
...
void QHexEdit::paintEvent(QPaintEvent *event) {
  ...
  if (event->rect() != _cursorRect) {
    ...
    if (_addressArea) {
      QString address;
      for (int row = 0, pxPosY = _pxCharHeight; row <= (_dataShown.size() / _bytesPerLine);
  ...
  if ((hexPositionInShowData >= 0) && (hexPositionInShowData < _hexDataShown.size())) {
    ...
    if (_readOnly) {
      ...
      painter.fillRect(QRect(_pxCursorX - pxOfsX,
